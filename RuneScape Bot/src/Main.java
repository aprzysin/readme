...
public void start() {
	...
	dutyList.addAll(Arrays.asList(new KillRat(ctx), new BurntDrop(ctx), new LumbridgeBank(ctx), new FoodBank(ctx), new LumbridgeRats(ctx), new Eater(ctx)));
	
}
@Override
	/**
	 * When logged in, this method keeps the code in it looping
	 */
public void poll() {
	// runs the execute method if the activate method returns true for each individual duty
